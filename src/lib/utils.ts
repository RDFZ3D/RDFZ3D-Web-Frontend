...
export const emptyAsNull = (value: string | null | undefined) => {
 ...
 */
export const getValidationTranslatedStringByKey = <
    TNamespace extends keyof TranslationFunctions,
>(
    ...
    if (!key) return;
    if (!key.startsWith("|:")) {return key as LocalizedString;}
    const realKey = key.slice(2);

    let field: LocalizedString | "" = "";
    if (fieldKey) {
        if (fieldKey in LL.common && LL.common[fieldKey as keyof typeof LL.common] instanceof (() => String)) {
